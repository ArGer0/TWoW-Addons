...
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
-- the "class" CharacterAbility
function statCompareClass.CharacterAbility()
	...
	function classMetatable:__tostring()
		local ret = "The character stat " .. self.name .. " has this values: \n";
	...
	function classInstance:compareValues()
		...
		if (self.actualValue and self.savedValue and self.actualValue.comparable) then
			...
			if (self.savedValue.detail or self.actualValue.detail) then
				if (self.savedValue.detail == nil) then
					ret.detail = self.actualValue.detail
				elseif (self.actualValue.detail == nil) then
					ret.detail = 0 - self.actualValue.detail;
				else
					ret.detail = self.actualValue.detail - self.savedValue.detail;
				end;
			end;
		end;
	...
	function classInstance:actualize()
		...
		-- load the ability from the ROM API
		local baseValue, extendValue = GetPlayerAbility(self.name);
		...
		ret = self.actualValue.value ~= math.ceil(baseValue + extendValue);
		ret = ret or self.actualValue.detail ~= math.ceil(baseValue);
		
		-- set the new values
		self.actualValue.value = math.ceil(baseValue + extendValue);
		self.actualValue.detail = math.ceil(baseValue);
...
statcompare.character.CharacterAbility = statCompareClass.CharacterAbility();
