		...
		for key, value in pairs(valuesToUpdate) do
			...
			if (fontInstance ~= nil) then
				...
				if (fontInstance:GetParent() == self.frameInstance) then
					...
					-- only actualize if there is a comparedValue present (there is no value if the first
					...
					if (value.actualValue and value.savedValue) then
						local compared = value:compareValues();
						if (compared and compared.toCompareString) then
							fontInstance:SetText(compared:toCompareString());
						end;
					end;
				end;
			end;
